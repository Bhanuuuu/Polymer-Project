      ...
      <app-route route="{{route}}" pattern="[[rootPath]]:page" data="{{routeData}}" tail="{{subroute}}">
        ...
        <!-- Main content -->
          ...
          <iron-pages selected="[[page]]" attr-for-selected="name" role="main">
            ...
            <product-details product-list="{{productList}}" product-id="{{productId}}" name="product"></product-details>
            <cart-items product-list="{{productList}}" product-id="{{productId}}" name="cart"></cart-items>
  ...
  static get properties() {
    return {
     ...
     name:{
      ...
      // productList which is used for storing the values
      productList:{
      type: Array,
      value : productList,
...
_productsListChanged() {
  ...
  _routePageChanged(page) {
    ...
    if (!page) {
      this.page = 'login';
    } else if (['login', 'home', 'product','cart','register'].indexOf(page) !== -1) {
      this.page = page;
    } else {
      this.page = 'view404';
    }
